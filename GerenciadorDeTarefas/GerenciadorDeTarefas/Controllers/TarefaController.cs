        ...
        public IActionResult AdicionarTarefa([FromBody] Tarefa tarefa)
        {
            try
            {
                ...
                else
                {
                    ...
                    if(tarefa.DataConclusao == DateTime.MinValue || tarefa.DataConclusao.Date < DateTime.Now.Date || tarefa.DataConclusao.Date > tarefa.DataPrevistaConclusao)
                    {
                        erros.Add("Data de conclusão não pode ser maior que a Data de previsão");
                    }
                }
